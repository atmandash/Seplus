...
if (!process.env.MONGODB_URI) {
    ...
    console.error('Please set MONGODB_URI in your environment or Render dashboard.');
} else {
    mongoose.connect(process.env.MONGODB_URI)
        .then(() => console.log('MongoDB Connected'))
        .catch(err => {
            console.error('MongoDB Connection Error:', err.message);
            // Don't exit process, let it stay alive so health checks might pass or we can see logs
        });
}
